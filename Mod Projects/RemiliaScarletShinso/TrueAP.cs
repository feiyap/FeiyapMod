...
namespace RemiliaScarletShinso
{
    public static class TrueAp
    {
        public static int TrueAP
        {
            ...
            set
            {
                SetTrueAP(value, false);
            }
        }

        private static int _TrueAP;

        public static void SetTrueAP(int value, bool NewTurnRecover = false)
        {
            foreach (IP_APChanged ip_APChanged in BattleSystem.instance.IReturn<IP_APChanged>())
            {
                if (ip_APChanged != null)
                {
                    ip_APChanged.APChanged(_TrueAP, value, NewTurnRecover);
                }
            }
            foreach (IP_TrueAPChanged ip_TrueAPChanged in BattleSystem.instance.IReturn<IP_TrueAPChanged>())
            {
                if (ip_TrueAPChanged != null)
                {
                    ip_TrueAPChanged.TrueAPChanged(_TrueAP, value, NewTurnRecover);
                }
            }
            _TrueAP = value;
            if (BattleSystem.instance.AllyTeam.AP + TrueAp.TrueAP >= 10)
            {
        ...
        public static void APChangeTrue(int value)
        {
            for (int i = 0; i < BattleSystem.instance.AllyTeam.AP && i < value; i++)
            {
    ...
    public interface IP_TrueAPChanged
    {
        void TrueAPChanged(int OldValue, int NewValue, bool NewTurnRecover);
    }

    [HarmonyPatch(typeof(BattleAlly))]
    public static class BattleAllyPatch
    {
        //释放普通技能时优先消耗法力值；释放真血技能时优先消耗真法力值
        [HarmonyPrefix]
    ...
    public static class BattleSystemPatch
    {
        //战斗开始时清空真法力值
        [HarmonyPrefix]
        [HarmonyPatch("BattleStart")]
        public static bool BattleStart_Prefix()
        {
            TrueAp.TrueAP = 0;
        ...
        public static bool BattleEnd_Prefix()
        {
            TrueAp.TrueAP = 0;
    ...
    public class BattleActWindowPatch
    {
        ...
        private static void Init_Postfix(BattleActWindow __instance, BattleTeam Team)
        {
            ...
            if (TrueAp.TrueAP > 0)
            {
                ...
                {
                    __instance.Crystals[i].transform.Find("On").GetComponent<Image>().color = new UnityEngine.Color(255 / 255f, 69 / 255f, 0f);
                }
            }
        }
    }
    ...
    public static class SkillButtonPatch
    {
        ...
        public static void Update_Postfix(SkillButton __instance)
        {
            ...
            if (BattleSystem.instance != null)
            {
                ...
                {
                    if ((__instance.Myskill.Master.MyTeam.AP + TrueAp.TrueAP) >= ap_OverloadViewOnly)
                    {
                        __instance.interactable = true;
                    }
                }
        ...
        public static bool OnPointerEnter_Prefix(SkillButton __instance)
        {
            ...
            {
                for (int i = (__instance.Myskill.MyTeam.AP + TrueAp.TrueAP); i > (__instance.Myskill.MyTeam.AP + TrueAp.TrueAP) - __instance.Myskill.AP; i--)
                {
