using ChronoArkMod;
namespace RemiliaScarlet
{
    public static class ModItemKeys
    {
		/// <summary>
		/// <sprite name="RemiliaScarlet_blood"><color=#FF3030>绯夜</color>
		/// 受到痛苦伤害时，每1层层数会治疗施加者5点体力。1回合最多触发1次。
		/// </summary>
        public static string Buff_B_RemiliaScarlet_0 = "B_RemiliaScarlet_0";
		/// <summary>
		/// 摇篮曲
		/// 受击时减少1层，赋予敌人1层[绯夜]。
		/// </summary>
        public static string Buff_B_RemiliaScarlet_1 = "B_RemiliaScarlet_1";
		/// <summary>
		/// 饱足
		/// 这场战斗中不能再从技能中获得永久最大生命值。
		/// </summary>
        public static string Buff_B_RemiliaScarlet_2 = "B_RemiliaScarlet_2";
		/// <summary>
		/// 红之异想
		/// 无法攻击&target以外的目标。
		/// 击中时不解除。
		/// </summary>
        public static string Buff_B_RemiliaScarlet_3 = "B_RemiliaScarlet_3";
		/// <summary>
		/// 爪击准备...
		/// </summary>
        public static string Buff_B_RemiliaScarlet_4 = "B_RemiliaScarlet_4";
		/// <summary>
		/// 吸血鬼之心
		/// </summary>
        public static string Buff_B_RemiliaScarlet_5 = "B_RemiliaScarlet_5";
		/// <summary>
		/// 饱足
		/// 蕾米莉亚从血肉中获得了满足。暂时无法提升最大生命值。
		/// </summary>
        public static string Buff_B_RemiliaScarlet_6 = "B_RemiliaScarlet_6";
		/// <summary>
		/// 千年吸血鬼
		/// 造成伤害25%吸血；每次造成伤害抽1个技能，最多触发4次。
		/// </summary>
        public static string Buff_B_RemiliaScarlet_LucyD = "B_RemiliaScarlet_LucyD";
		/// <summary>
		/// 永幼之赤月
		/// 增加10%暴击率、命中率、闪避率。
		/// </summary>
        public static string Buff_B_RemiliaScarlet_P = "B_RemiliaScarlet_P";
		/// <summary>
		/// 蕾米莉亚
		/// Passive:
		/// 蕾米莉亚能够操纵命运，增加所有队友10%暴击率、命中率、闪避率。
		/// 此外，每个敌人阵亡时，增加蕾米莉亚1点永久最大体力值。最多增加当前等级x5点。
		/// （该被动在1级即生效。）
		/// </summary>
        public static string Character_RemiliaScarlet = "RemiliaScarlet";
        public static string Character_Skin_RemiliaScarletEclipse = "RemiliaScarletEclipse";
        public static string Character_Skin_RemiliaScarletLord = "RemiliaScarletLord";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_Hein = "CampDial_RemiliaScarlet_Hein";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_Joey = "CampDial_RemiliaScarlet_Joey";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_Sizz = "CampDial_RemiliaScarlet_Sizz";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_Trisha = "CampDial_RemiliaScarlet_Trisha";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_Miss = "CampDial_RemiliaScarlet_Miss";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_Azar = "CampDial_RemiliaScarlet_Azar";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_Lian = "CampDial_RemiliaScarlet_Lian";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_Pheonix = "CampDial_RemiliaScarlet_Pheonix";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_Priest = "CampDial_RemiliaScarlet_Priest";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_Iron = "CampDial_RemiliaScarlet_Iron";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_Charon = "CampDial_RemiliaScarlet_Charon";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_SilverStein = "CampDial_RemiliaScarlet_SilverStein";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_Huz = "CampDial_RemiliaScarlet_Huz";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_Red = "CampDial_RemiliaScarlet_Red";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_Blue = "CampDial_RemiliaScarlet_Blue";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_Narhan = "CampDial_RemiliaScarlet_Narhan";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_Johan = "CampDial_RemiliaScarlet_Johan";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_Ilya = "CampDial_RemiliaScarlet_Ilya";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_Leryn = "CampDial_RemiliaScarlet_Leryn";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_Momori = "CampDial_RemiliaScarlet_Momori";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_Patchouli = "CampDial_RemiliaScarlet_Patchouli";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_Mokou = "CampDial_RemiliaScarlet_Mokou";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_IzayoiSakuya = "CampDial_RemiliaScarlet_IzayoiSakuya";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_FlandreScarlet = "CampDial_RemiliaScarlet_FlandreScarlet";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_SatsukiRin = "CampDial_RemiliaScarlet_SatsukiRin";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_Youmu = "CampDial_RemiliaScarlet_Youmu";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_Reisen = "CampDial_RemiliaScarlet_Reisen";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_TouhouAlice = "CampDial_RemiliaScarlet_TouhouAlice";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_Eirin = "CampDial_RemiliaScarlet_Eirin";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_HouraisanKaguya = "CampDial_RemiliaScarlet_HouraisanKaguya";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_Cirno = "CampDial_RemiliaScarlet_Cirno";
        public static string SimpleCampDialogue_CampDial_RemiliaScarlet_Daiyousei = "CampDial_RemiliaScarlet_Daiyousei";
		/// <summary>
		/// 特别定制的太阳伞
		/// 当装备者给目标施加痛苦或弱化减益时，治疗自己10%最大体力。
		/// </summary>
        public static string Item_Equip_Remilia_Parasol = "Remilia_Parasol";
        public static string SkillEffect_SE_S_S_RemiliaScarlet_10Rare = "SE_S_S_RemiliaScarlet_10Rare";
        public static string SkillEffect_SE_S_S_RemiliaScarlet_12Rare = "SE_S_S_RemiliaScarlet_12Rare";
        public static string SkillEffect_SE_S_S_RemiliaScarlet_3 = "SE_S_S_RemiliaScarlet_3";
        public static string SkillEffect_SE_S_S_RemiliaScarlet_4 = "SE_S_S_RemiliaScarlet_4";
        public static string SkillEffect_SE_Tick_B_RemiliaScarlet_0 = "SE_Tick_B_RemiliaScarlet_0";
        public static string SkillEffect_SE_T_S_RemiliaScarlet_0 = "SE_T_S_RemiliaScarlet_0";
        public static string SkillEffect_SE_T_S_RemiliaScarlet_1 = "SE_T_S_RemiliaScarlet_1";
        public static string SkillEffect_SE_T_S_RemiliaScarlet_10 = "SE_T_S_RemiliaScarlet_10";
        public static string SkillEffect_SE_T_S_RemiliaScarlet_12Rare = "SE_T_S_RemiliaScarlet_12Rare";
        public static string SkillEffect_SE_T_S_RemiliaScarlet_2 = "SE_T_S_RemiliaScarlet_2";
        public static string SkillEffect_SE_T_S_RemiliaScarlet_3 = "SE_T_S_RemiliaScarlet_3";
        public static string SkillEffect_SE_T_S_RemiliaScarlet_4 = "SE_T_S_RemiliaScarlet_4";
        public static string SkillEffect_SE_T_S_RemiliaScarlet_5 = "SE_T_S_RemiliaScarlet_5";
        public static string SkillEffect_SE_T_S_RemiliaScarlet_6 = "SE_T_S_RemiliaScarlet_6";
        public static string SkillEffect_SE_T_S_RemiliaScarlet_7 = "SE_T_S_RemiliaScarlet_7";
        public static string SkillEffect_SE_T_S_RemiliaScarlet_8 = "SE_T_S_RemiliaScarlet_8";
        public static string SkillEffect_SE_T_S_RemiliaScarlet_9 = "SE_T_S_RemiliaScarlet_9";
        public static string SkillEffect_SE_T_S_RemiliaScarlet_LucyD = "SE_T_S_RemiliaScarlet_LucyD";
        public static string SkillEffect_SE_T_S_RemiliaScarlet_P = "SE_T_S_RemiliaScarlet_P";
		/// <summary>
		/// 命中时，自身获得相当于伤害值25%的超额治疗
		/// 攻击技能
		/// </summary>
        public static string SkillExtended_SkillEn_Remilia_0 = "SkillEn_Remilia_0";
		/// <summary>
		/// 红符「红色不夜城」
		/// 命中后使目标身上的<sprite name="RemiliaScarlet_blood"><color=#FF3030>绯夜</color>立即结算一次伤害，并将<sprite name="RemiliaScarlet_blood"><color=#FF3030>绯夜</color>延长1回合。
		/// </summary>
        public static string Skill_S_RemiliaScarlet_0 = "S_RemiliaScarlet_0";
		/// <summary>
		/// 必杀「Heart Break」
		/// 依据自身最大体力值，额外造成&a(20%)伤害。这个技能的100%将会超额治疗自己。
		/// </summary>
        public static string Skill_S_RemiliaScarlet_1 = "S_RemiliaScarlet_1";
		/// <summary>
		/// 「绯红色的幻想乡」
		/// </summary>
        public static string Skill_S_RemiliaScarlet_10Rare = "S_RemiliaScarlet_10Rare";
		/// <summary>
		/// 神术「吸血鬼幻想」
		/// 本回合内技能交换次数增加10次。抽2个技能。
		/// </summary>
        public static string Skill_S_RemiliaScarlet_11Rare = "S_RemiliaScarlet_11Rare";
		/// <summary>
		/// 红魔「Scarlet Devil」
		/// </summary>
        public static string Skill_S_RemiliaScarlet_12Rare = "S_RemiliaScarlet_12Rare";
		/// <summary>
		/// 神枪「Spear the Gungnir」
		/// 依据自身最大体力值，额外造成&a(40%)伤害。使用此技能击杀后增加2点永久最大生命值，恢复2点行动力，这场战斗中无法再从技能中获取永久最大生命值。
		/// </summary>
        public static string Skill_S_RemiliaScarlet_2 = "S_RemiliaScarlet_2";
		/// <summary>
		/// 夜符「Demon King Cradle」
		/// 同时攻击所有附带[绯夜]减益的敌人。
		/// 每个受到攻击的敌人会使自身获得1层[摇篮曲]。
		/// 如果一次攻击到3个或以上的单位，获得1层[吸血鬼之心]。
		/// </summary>
        public static string Skill_S_RemiliaScarlet_3 = "S_RemiliaScarlet_3";
		/// <summary>
		/// 夜王「Dracula Cradle」
		/// </summary>
        public static string Skill_S_RemiliaScarlet_4 = "S_RemiliaScarlet_4";
		/// <summary>
		/// 恶魔「Remilia Stretch」
		/// 依据自身最大体力值，额外造成&a(50%)伤害。倒计时期间，自身获得50%受伤减免和100%减益抵抗。
		/// </summary>
        public static string Skill_S_RemiliaScarlet_5 = "S_RemiliaScarlet_5";
		/// <summary>
		/// 夜符「Bad Lady Scramble」
		/// </summary>
        public static string Skill_S_RemiliaScarlet_6 = "S_RemiliaScarlet_6";
		/// <summary>
		/// 狱符「千根针的针山」
		/// 这个技能造成伤害的250%将会超额治疗自己。
		/// </summary>
        public static string Skill_S_RemiliaScarlet_7 = "S_RemiliaScarlet_7";
		/// <summary>
		/// 诅咒「弗拉德·特佩斯的诅咒」
		/// 仅在自身拥有[吸血鬼之心]增益的时候才可使用。
		/// 消耗1层[吸血鬼之心]增益，超额治疗&a(自身20%最大体力值)的体力。
		/// </summary>
        public static string Skill_S_RemiliaScarlet_8 = "S_RemiliaScarlet_8";
		/// <summary>
		/// 命运「Miserable Fate」
		/// 同时攻击1个目标以外的随机敌人。
		/// </summary>
        public static string Skill_S_RemiliaScarlet_9 = "S_RemiliaScarlet_9";
		/// <summary>
		/// 「千年吸血鬼」
		/// </summary>
        public static string Skill_S_RemiliaScarlet_LucyD = "S_RemiliaScarlet_LucyD";

    }

    public static class ModLocalization
    {

    }
}